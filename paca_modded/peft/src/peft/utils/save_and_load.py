...
def get_peft_model_state_dict(
    ...
    elif config.peft_type in (PeftType.PACA):
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "paca_" in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if "paca_" in k or "bias" in k}
        else:
            raise NotImplementedError
        to_return = {k: v for k, v in to_return.items() if (("paca_" in k and adapter_name in k) or ("bias" in k))}
