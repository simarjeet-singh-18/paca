...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        self.moslora_layer = nn.ModuleDict({})
        # For Embedding layer
    ...
    def update_layer(
        ...
        if use_moslora:
            self.moslora_init(r, adapter_name)
    ...
    def moslora_init(self, r, adapter_name: str) -> None:
        if not self.lora_magnitude_vector:
            # first dora layer being added, add lora_magnitude_vector to the list of learnable parameters
            self.adapter_layer_names = self.adapter_layer_names[:] + ("lora_magnitude_vector",)

        self.moslora_layer[adapter_name] = nn.Linear(r,r, bias=False)
...
class Linear(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.use_moslora[adapter]:
            weight_moslora = self.moslora_layer[adapter].weight
        
        if cast_to_fp32:
            weight_A = weight_A.float()
            weight_B = weight_B.float()
            if self.use_moslora[adapter]:
                weight_moslora =weight_moslora.float()
        
        if not(self.use_moslora[adapter]):
            output_tensor = transpose(weight_B @ weight_A, self.fan_in_fan_out) * self.scaling[adapter]
        else:
            output_tensor = transpose(weight_B @ weight_moslora @ weight_A, self.fan_in_fan_out) * self.scaling[adapter]    
        ...
        if cast_to_fp32:
            ...
            if self.use_moslora[adapter]:
                self.moslora_layer[adapter].weight.data = weight_moslora.to(dtype)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if self.use_moslora[active_adapter]:
                    moslora_layer=self.moslora_layer[active_adapter]
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                x = x.to(lora_A.weight.dtype)

                if not (self.use_dora[active_adapter] or self.use_moslora[active_adapter]):
                    result = result + lora_B(lora_A(dropout(x))) * scaling
                elif self.use_moslora[active_adapter]:
                    result = result + lora_B(moslora_layer(lora_A(dropout(x)))) * scaling
                else:
                    x = dropout(x)
