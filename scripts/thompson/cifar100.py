#!/usr/bin/env python3
"""
Vision Transformer (ViT) Fine-tuning Script for CIFAR-100 with Thompson Sampling
"""
import os, time
import torch
...
def create_dataloaders():
    ...
    # Download and load CIFAR100 dataset directly from torchvision
    ...
    # Get number of classes (CIFAR-100 has 100 classes)
    num_classes = 100
...
print("=" * 60)
print("Vision Transformer Fine-tuning on CIFAR-100 with Thompson Sampling")
print("=" * 60)
...
# Configure PaCA with Thompson Sampling
peft_config = PacaConfig(
    ...
    arm_selection_method="thompson",  # Use Thompson Sampling instead of UCB
    thompson_alpha_prior=1.0,         # Beta distribution prior for successes
    thompson_beta_prior=1.0,          # Beta distribution prior for failures
)
...
def train(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, patience=7):
    ...
    for epoch in range(epochs):
        ...
        # Calculate reward for Thompson Sampling
        if prev_val_acc is not None:
            reward = val_acc - prev_val_acc
            if epoch < 2: 
                reward = 0
            model.update_ucb_rewards("default", reward)
            print(f"\nThompson Sampling Reward: {reward:.2f}%")
        
        prev_val_acc = val_acc
        
        # Reselect arms using Thompson Sampling
...
print("\n" + "=" * 60)
print("Fine-tuning ViT using Thompson Sampling-based PaCA:")
print("=" * 60)

# First training run with Thompson Sampling
