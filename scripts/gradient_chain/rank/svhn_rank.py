#!/usr/bin/env python3
"""
Vision Transformer (ViT) Fine-tuning Script for SVHN with Gradient Chain Selection
Uses gradient-based chain construction to create neural pathways through the network.
Takes rank as a command line input.
"""
import os, time
import argparse
import torch
...
# Argument Parsing
parser = argparse.ArgumentParser(description="ViT Fine-tuning on SVHN with Gradient Chain Selection")
parser.add_argument("--rank", type=int, default=8, help="Rank (r) for PaCA")
...
SAVE_DIR = os.path.join(PROJECT_ROOT, "saved_models_pretrained", f"vit_svhn_gradient_chain_r{args.rank}")
...
def create_dataloaders():
    ...
    # SVHN uses 'split' argument instead of 'train'
...
print(f"Using Gradient Chain Selection - Rank {args.rank}")
print("=" * 60)
...
peft_config = PacaConfig(
    ...
    paca_alpha=args.rank * 2, # Scaling alpha with rank
    bias="all",
