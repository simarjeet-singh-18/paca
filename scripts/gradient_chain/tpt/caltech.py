#!/usr/bin/env python3
"""
Vision Transformer (ViT) Throughput Measurement Script for Caltech-101

This script measures and compares the training throughput (images/sec) of
Full Fine-Tuning, LoRA, and PaCA (Gradient Chain) on the Caltech-101 dataset.
"""
import os
import time
import torch
...
def create_dataloader(batch_size):
    ...
    return train_loader, len(full_dataset), num_classes
...
def create_model(num_classes=100, method='Full_FT', trainloader=None):
    ...
    if method == 'LoRA':
        ...
        config = PacaConfig(
            ...
            gradient_chain_warmup_epochs=2,
...
def plot_results(results):
    ...
    ax.set_title("ViT Fine-Tuning Throughput on Caltech-101 (Gradient Chain)", fontsize=16)
    ...
    plt.tight_layout()
    save_path = "throughput_gradient_chain_caltech.png"
...
def main():
    ...
    print("=" * 60)
    print("Starting Throughput Analysis for ViT on Caltech-101")
    ...
    for method in METHODS_TO_TEST:
        ...
        for batch_size in BATCH_SIZES_TO_TEST:
            ...
            try:
                train_loader, num_samples, num_classes = create_dataloader(batch_size)
                model = create_model(num_classes=num_classes, method=method, trainloader=train_loader)
