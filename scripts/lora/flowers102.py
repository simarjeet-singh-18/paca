...
# flowers102
"""
Vision Transformer (ViT) Fine-tuning Script for Caltech-101 Dataset using LoRA
"""
...
SAVE_DIR = "../../saved_models_pretrained/vit_flowers"
...
def create_dataloaders():
    ...
    #    We load them all with the training transform initially.
    ...
    #    This is SUPER important so you get the same split every time.
    torch.manual_seed(42)
    train_dataset, test_dataset = random_split(full_dataset, [train_size, test_size])

    # 5. IMPORTANT: The test_dataset subset currently points to images with training transforms.
    #    We need to ensure the test set uses the validation transform (no data augmentation).
    #    We do this by changing the transform on the underlying dataset for the test subset.
    #    This is a bit of a hack, but it's the simplest way to ensure correct evaluation.
    #
    #    We create a new ConcatDataset for the test set with the correct transform.
...
print("=" * 60)
print("Vision Transformer Fine-tuning on Flowers102")
print("=" * 60)
...
model = get_peft_model(model_vanilla, peft_config)
print(f"LoRA model summary: {model}")
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            if epochs_no_improve >= patience:
                ...
                print(f"Total training time so far: {total_time:.2f} seconds")
                print(f"Average time per epoch: {total_time / (epoch + 1):.2f} seconds")
                break

    return model

print("Fine-tuning ViT using LoRA:")
